  ...
  foreach ($_POST as $the_key => $the_post) {
    switch ($the_key) {
      case "fullname":
        ...
        try {
          $sth->execute([$uname, Auth::user()->old_user->login]);
      ...
      case "mail":
        ...
        try {
          $sth->execute([$umail, Auth::user()->old_user->login]);
        } catch (PDOException $e) {
          echo $e->getMessage();
        }
        $notif = [
      ...
      case "rpps":
        ...
        try {
          $sth->execute([$urpps, Auth::user()->old_user->login]);
        } catch (PDOException $e) {
          echo $e->getMessage();
        }
        $notif = [
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="col-xs-6 col-xs-offset-3">
        ...
        <form class="form" method="POST" action="<?php echo $_SERVER[
          "PHP_SELF"
        ]; ?>">
        <?php endif; ?>
        <div class="row">
          <div class="col-xs-10">
            ...
            <?php if ($the_info == $_GET["edit"]): ?>
              <input type="text" class="form-control" name="<?php echo $the_info; ?>" value="<?php echo $user[
  $the_info
]; ?>"/>
            <?php else: ?>
          ...
          <div class="col-xs-2">
            <?php if ($the_info != $_GET["edit"]): ?>
              ...
              <a href="<?php echo $_SERVER[
                "PHP_SELF"
              ]; ?>?edit=<?php echo $the_info; ?>" class="btn btn-xs btn-danger">Modifier</a>
            <?php else: ?>
              <br/><br/>
