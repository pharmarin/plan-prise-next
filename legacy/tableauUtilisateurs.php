...
?>
     ...
     for ($i = 0; $i < 12; $i++): ?>
					...
					<?php $month = $month == 1 ? 12 : $month - 1;endfor;
     ?>
				</tr>
    ...
    require "connexion.php";
    $query =
      "SELECT COUNT(*) AS COUNT, MONTH(TIME) AS MONTH FROM calendriers WHERE TIME > LAST_DAY(SUBDATE(CURRENT_DATE(), INTERVAL 1 YEAR)) GROUP BY MONTH(TIME)";
    $sth = $dbh->query($query);
    $row = $sth->fetchAll(PDO::FETCH_ASSOC);
    foreach ($row as $key => $value) {
      $calendriers[$value["MONTH"]] = $value["COUNT"];
    }

//var_dump($calendriers);
?>
				<tr>
					<td>
						Calendriers
     ...
     for ($i = 0; $i < 12; $i++): ?>
						<td class="text-center">
							<?php echo isset($calendriers[$month]) ? $calendriers[$month] : 0; ?>
						</td>
					<?php $month = $month == 1 ? 12 : $month - 1;endfor;
     ?>
				</tr>
...
?>
        ...
        echo "Actif";
         ...
         case 2:
           ...
           break;
						...
						<td><?= $row["rpps"] ?></td>
						<td><a href='ajoutUser.php?supprime=<?= $row[
        "Id"
      ] ?>' onClick="confirmation(event, '<?= $row[
  "fullname"
  ...
  "fullname"
     ...
     ?>
			...
			<ul>
				<li><p><a href="saisieUser.php">Ajouter un utilisateur</a></p></li>
			</ul>
		</div>
	</body>
	<script>
