...
if ($_GET["modifier"] == "modifier") {
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    if ($comment[$i] == "UPDATE") {
      $query =
        "UPDATE precautions SET mot_cle='" .
        $mot_cle[$i] .
        "', titre='" .
        $titre[$i] .
        "',
      ...
      $query =
        "INSERT INTO precautions (mot_cle, titre, contenu, couleur) VALUES ('" .
        $mot_cle[$i] .
        "', '" .
        $titre[$i] .
        "',
   ...
   if ($i != 0) {
     echo "<hr>";
         ...
         echo 'class="clone"';
			    	...
			    	<input type="hidden" class="id" name="id[]" value="<?php echo $rows[
          "id"
        ]; ?>">
			    	<input type="hidden" class="comment" name="comment[]" value="UPDATE">
			    	<div class="mot">
			    		<label for="mot_cle[]">Mot clé : </label>
			    		<input class="form-control input-sm prec-mot" name="mot_cle[]" type="text" value="<?php echo $rows[
           "mot_cle"
         ]; ?>">
			    	</div>
					<div class="panel panel-warning" style="border-color:#e7e7e7">
						<div class="panel-heading" style="background-color:<?php echo $rows[
        "couleur"
      ]; ?> !important;color:white;border-color:#e7e7e7">
							<input class="form-control input-sm prec-titre" name="titre[]" type="text" value="<?php echo $rows[
         "titre"
       ]; ?>">
							<label for="couleur[]">Couleur : </label>
							<input class="form-control input-sm couleur" type="color" name="couleur[]" value="<?php echo $rows[
         "couleur"
       ]; ?>">
						</div>
