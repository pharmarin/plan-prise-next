...
if (isset($action)) {
  if ($action == "Ajouter") {
    ...
    foreach ($data as $key => $row) {
      if ($row["id_medic"] == $_POST["id_medic"]) {
        $data[$key] = $_POST;
        calendar_update($data, $id);
      }
    }
...
if (isset($_POST["modifier"]) && !empty($_POST["modifier"])) {
  foreach ($data as $value) {
    if ($value["id_medic"] == $_POST["modifier"]) {
      $modifier = $value;
    }
  }
}
...
if (isset($_POST["supprimer"]) && !empty($_POST["supprimer"])) {
  foreach ($data as $key => $value) {
    if ($value["id_medic"] == $_POST["supprimer"]) {
      unset($data[$key]);
      calendar_update($data, $id);
    }
  }
					...
					<tr>
						<td>
							<div class="wrapper">
								...
								<?php if (isset($erreur)) { ?>
									<div class="alert alert-xs alert-margin text-center alert-<?= $erreur[
           "level"
         ] ?>"><?= $erreur["text"] ?></div>
								<?php } ?>
										...
										<div class="form-group col-xs-12">
											<select class="form-control" style="width:100%" name="id_medic" multiple="multiple">
												...
												<?php if (isset($erreur)) { ?>
													<option value="<?= $_POST[
               "id_medic"
             ] ?>" selected="selected"><?php echo ctype_digit(
  $_POST["id_medic"]
)
  ...
  echo 'value="' . $_POST["dates"][$i] . '"';
        ...
        ?>
          ...
          : $modifier["id_medic"]; ?></h4>
								...
								<form action="<?= $_SERVER[
          "REQUEST_URI"
        ] ?>" method="POST" class="form-inline box-wrapper">
        ...
        for ($i = 0; $i < $nb; $i++) {
          ...
          ?>
									<div class="formulaire">
										<div class="row">
											<div class="form-group col-xs-12 unites">
												<input id="nombre" name="nombre[]" type="text" min="0" class="form-control input-sm" value="<?= $modifier[
              "nombre"
            ][$i] ?>">
												<label> unité(s) de prise tous les </label>
												<input id="frequence" name="frequence[]" type="text" min="1" class="form-control input-sm" value="<?= $modifier[
              "frequence"
            ][$i] ?>">
												<label> jours</label>
												<a href="#" class="btn btn-round btn-xs btn-danger remove-period" data-toggle="tooltip" data-placement="left" title="Supprimer cette période"><span class="glyphicon glyphicon-remove"></span></a>
										...
										<div class="row">
											<div class="form-group col-xs-12 periode">
												<input name="dates[]" type="text" id="date<?= $datecount ?>" class="form-control input-sm hidden" value="<?= $modifier[
  "dates"
][$i] ?>">
        ...
        ?>
									...
									<div class="text-center action">
										...
										<a href="<?= $_SERVER[
            "REQUEST_URI"
          ] ?>" class="btn btn-round btn-danger btn-lg">Annuler</a>
									</div>
								</form>
							</div>
						</td>
         ...
         if (
          ...
          if ($i != 0) {
            echo "<hr/>";
									...
									<div class="row">
										<div class="col-xs-12">
											<p><?= $entree["nombre"][$i] ?>  unité(s) de prise tous les <?= $entree[
    "frequence"
  ][$i] ?> jours</p>
										</div>
									</div>
								...
								<?php
									...
									<div class="text-center action">
										<form action="" method="POST">
											<input type="hidden" name="id_cal" value="<?= $id ?>">
											<button type="submit" name="modifier" class="btn btn-round btn-warning btn-lg" value="<?= $entree[
             "id_medic"
           ] ?>"><span class="glyphicon glyphicon-edit"></span> Modifier</button>
											<button type="submit" name="supprimer" class="btn btn-round btn-danger btn-lg" value="<?= $entree[
             "id_medic"
