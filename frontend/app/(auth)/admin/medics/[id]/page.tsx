...
const Medicament = async ({ params }: { params: { id: string } }) => {
  try {
    ...
    return (
      <>
        <Navigation
          title={medicament.denomination}
          returnTo={"/admin/medics"}
          options={[{ icon: "edit", path: `/admin/medics/${params.id}/edit` }]}
        />
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <TypographyList>
              {extractVoieAdministration(medicament).map(
                (voieAdministration) => (
          ...
          <div className="space-y-1">
            ...
            <TypographyList>
              {(medicament.conservationDuree ?? []).map(
                (conservation, index) => (
                  <li key={index}>
                  ...
                  <CardContent className="mt-6">
                    ...
                    {commentaire.population && (
                      <p className="italic text-muted-foreground">
                        Concerne la population suivante :{" "}
                        {commentaire.population}
                      </p>
                    )}
